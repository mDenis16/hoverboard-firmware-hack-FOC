...
int16_t cur_phaA = 0, cur_phaB = 0, cur_DC = 0;
...
void DMA1_Channel1_IRQHandler(void) {
  ...
  // Get motor currents
  cur_phaA = (int16_t)(offsetrlA - adc_buffer.rlA);
  cur_phaB = (int16_t)(offsetrlB - adc_buffer.rlB);
  cur_DC   = (int16_t)(offsetdcl - adc_buffer.dcl);
  

  // Disable PWM when current limit is reached (current chopping)
  // This is the Level 2 of current protection. The Level 1 should kick in first given by I_MOT_MAX
  if(ABS(cur_DC) > curDC_max || enable == 0) {
  ...
  // ========================= RIGHT MOTOR ===========================  
    ...
    rtU.b_hallB       = hall_vr;
    rtU.b_hallC       = hall_wr;
    rtU.i_phaAB       = cur_phaA;
    rtU.i_phaBC       = cur_phaB;
    rtU.i_DCLink      = cur_DC;
